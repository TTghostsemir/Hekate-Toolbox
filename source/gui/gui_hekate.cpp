...
static void reboot_to_payload(void) {
    ...
    if (R_FAILED(rc)) {
        printf("Neustart payload konnte nicht gesetzt werden: 0x%x\n", rc);
    }
    else {
        spsmShutdown(true);
    }
...
GuiHekate::GuiHekate() : Gui() {
    ...
    if (R_SUCCEEDED(rc = setsysInitialize())) {
        ...
        if (model != SetSysProductModel_Nx && model != SetSysProductModel_Copper) {
            ...
            if (model == SetSysProductModel_Iowa || model == SetSysProductModel_Hoag || model == SetSysProductModel_Calcio || model == SetSysProductModel_Aula) {
                errorMessage = "Nicht unterstuetztes Switch-Modell (Mariko)";
            }
            else {
                errorMessage = "Unbekanntes switch model";
            }
    ...
    if (canReboot && R_FAILED(rc = spsmInitialize())) {
        ...
        restrictedMode = true;
        canReboot = false;
        errorMessage = "Initialisierung von spsm fehlgeschlagen!";
    }
    ...
    if (canReboot && !restrictedMode) {
        ...
        if (R_FAILED(rc = amsBpcInitialize())) {
            restrictedMode = true;
            errorMessage = "Initialisierung von ams:bpc fehlgeschlagen!";
            smInitialize();
        }
    }
    ...
    if (canReboot && !restrictedMode) {
        ...
        if (f == NULL) {
            restrictedMode = true;
            errorMessage = "Konnte \"bootloader/update.bin\" nicht Finden!";
        }
    }
...
void GuiHekate::initializeForRestrictedMode() {
    ...
    shutdownButton->drawAction = [shutdownButton](Gui *gui, u16 x, u16 y, bool *isActivated) {
        ...
        gui->drawTextAligned(font20, x + shutdownButton->volume.first / 2, y + shutdownButton->volume.second / 2 + 10, currTheme.textColor, "Ausschalten", ALIGNED_CENTER);
    };
    ...
    rebootButton->drawAction = [rebootButton](Gui *gui, u16 x, u16 y, bool *isActivated) {
        ...
        gui->drawTextAligned(font20, x + rebootButton->volume.first / 2, y + rebootButton->volume.second / 2 + 10, currTheme.textColor, "Neustarten zur payload", ALIGNED_CENTER);
    };
...
void GuiHekate::InitializeRegular() {
    ...
    m_rebootConfigs.push_back({"UMS Starten (SD Card)", 0, false, true});
    ...
    profileButton->inputAction = [&](u32 kdown, bool *isActivated) {
        if (kdown & HidNpadButton_A) {
            ...
            (new ListSelector("Hekate-Profil zum Neustart", "\uE0E1 Zurueck     \uE0E0 OK", rebootNames, currRebootEntryIndex))
    ...
    rebootButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        ...
        gui->drawTextAligned(font20, Gui::g_framebuffer_width / 2, y + 50, currTheme.textColor, "Jetzt Neustarten!", ALIGNED_CENTER);
    };
    rebootButton->inputAction = [&](u32 kdown, bool *isActivated) {
        if (kdown & HidNpadButton_A) {
            FILE *f = fopen("sdmc:/bootloader/update.bin", "rb");
            if (f) {
                ...
                (new MessageBox("Konnte \"bootloader/update.bin\" nicht Finden!", MessageBox::OKAY))->show();
        }
    };
    add(rebootButton);
}
...
void GuiHekate::draw() {
    ...
    if (canReboot)
    {
        if (restrictedMode)
        {
            Gui::drawTextAligned(font24, Gui::g_framebuffer_width / 2, 150, currTheme.activatedColor, "Beschraenkter Modus", ALIGNED_CENTER);
            Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 200, currTheme.textColor, "Ein direkter Neustart nach hekate ist nicht moeglich,\naber Sie koennen eine der beiden folgenden Optionen waehlen:", ALIGNED_CENTER);

            const std::string restrictedModeReason = "Beschraenkter Modus Grund: " + errorMessage;
        ...
        {
            Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 150, currTheme.textColor, "Waehlen Sie das Hekate-Profil aus, mit dem Sie Ihre Nintendo Switch neu starten moechten. \n Stellen Sie sicher, dass Sie vorher alle geoeffneten Titel schliessen, da Ihr Gerät dadurch sofort neu gestartet wird.", ALIGNED_CENTER);
        }
    }
    else
    {
