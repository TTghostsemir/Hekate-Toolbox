...
GuiOverrideKey::GuiOverrideKey() : Gui() {
    ...
    keySelectButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        if (m_inputBlocked)
            gui->drawTextAligned(font20, x + 190, y + 100, currTheme.textColor, "Druecke eine Beliebige Taste...", ALIGNED_CENTER);
        else if (m_override.key.key == 0)
            gui->drawTextAligned(font24, x + 190, y + 100, currTheme.unselectedColor, "Keine Taste Ausgewaelt", ALIGNED_CENTER);
        else
            gui->drawTextAligned(fontHuge, x + 190, y + 136, currTheme.textColor, OverrideKey::KeyToUnicode(m_override.key.key), ALIGNED_CENTER);
        gui->drawTextAligned(font14, x + 190, y + 185, currTheme.textColor, "Override Taste", ALIGNED_CENTER);
    };
    ...
    comboPressedButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        gui->drawTextAligned(font20, x + 20, y + 50, currTheme.textColor, "Die Taste muss:", ALIGNED_LEFT);
        gui->drawTextAligned(font20, x + 360, y + 50, currTheme.selectedColor, m_override.key.overrideByDefault ? "Nicht Gehalten werden" : "Gehalten werden", ALIGNED_RIGHT);
    };
    ...
    switch (g_keyType) {
        case OverrideKeyType::Any_App_Override: {
            ...
            anyTitleIconButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                gui->drawTextAligned(font24, x, y - 60, currTheme.textColor, "Override beim Starten:", ALIGNED_LEFT);
                gui->drawTextAligned(font24, x, y - 20, currTheme.textColor, "Beliebiges title", ALIGNED_LEFT);
            };
            ...
            overrideEnabledButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                gui->drawTextAligned(font20, x + 20, y + 50, currTheme.textColor, "Aktieviert", ALIGNED_LEFT);
                gui->drawTextAligned(font20, x + 360, y + 50, m_overrideAnyApp ? currTheme.selectedColor : currTheme.unselectedColor, m_overrideAnyApp ? "An" : "Aus", ALIGNED_RIGHT);
            };
            ...
            appIconButton->drawAction = [&, title{DumpTitle(m_override.programID)}](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                if (title.get() != nullptr && title->application_id != 0) {
                    ...
                    if (appletName == nullptr) {
                        ...
                        gui->drawTextAligned(font20, Gui::g_framebuffer_width / 2, y + 350, currTheme.activatedColor, "Das Oeffnen von hbmenu ueber Applet kann dazu führen, dass nicht genuegend Speicher für Homebrew-Anwendungen zur Verfuegung steht. \n Es wird empfohlen, hbmenu ueber ein Spiel heraus zu oeffnen, um vollen RAM-Zugriff zu erhalten.", ALIGNED_CENTER);
                    }
                    ...
                    gui->drawTextAligned(font24, x + 150, y + 280, currTheme.unselectedColor, "Kein title Ausgewaelt", ALIGNED_CENTER);
                }
            };
